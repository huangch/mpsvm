...
%  Author: Steve Gunn (srg@ecs.soton.ac.uk)
  ...
  if (nargin < 5 | nargin > 10) % check correct number of arguments
    ...
    gpuResult = double(matCudaM3SVM('predict', single(gpuInput), int32(size(gpuInput, 1)), single(X), single(Y), single(alpha), single(bias), int32(size(X,1)), int32(k), single(p1), single(p2), single(EPS), int32(wrap1), int32(wrap2)));
    matCudaM3SVM('release');    
    clear mex;
    
    for x1 = 1 : size(x,1)
      for y1 = 1 : size(x,2)
        z(x1,y1) = gpuResult((x1-1)*size(x,2)+y1);
        end
      end
      waitbar((x1)/size(x,1))
    end
